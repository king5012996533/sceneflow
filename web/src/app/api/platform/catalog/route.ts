 ...
 *
 * 汇总所有「启用」凭证的模型绑定，附上逐模型能力标定（不含任何 Key）。
...
export async function GET(req: NextRequest) {
    ...
    if (!prisma) return NextResponse.json({ models: [], updatedAt: null });
    ...
    try {
        ...
        return NextResponse.json({ models, updatedAt: new Date().toISOString() });
    } catch (error) {
