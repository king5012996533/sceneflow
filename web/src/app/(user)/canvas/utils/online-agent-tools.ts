...
const BASE_ONLINE_AGENT_PROMPT = [
    ...
    "用户说基于图片生成视频、让图片动起来、图生视频时，必须先读取选区或画布状态，再创建图生视频流程或生成节点。",
...
export const ONLINE_AGENT_TOOLS: ResponseFunctionTool[] = [
    ...
    toolDefinition("canvas_create_reverse_prompt_flow", "基于指定图片节点创建反推提示词流程：参考图 -> 反推说明文本 -> 文本生成配置，可选择立即运行。用户要求从图片反推/倒推/提取提示词时优先使用。", { nodeId: { type: "string" }, brief: { type: "string" }, x: { type: "number" }, y: { type: "number" }, autoRun: { type: "boolean" } }, ["nodeId"]),
    ...
    toolDefinition("canvas_run_pipeline", "按顺序执行一组已确认的流程节点。成功节点会跳过，失败时停止，再次调用可以从断点继续。该工具会消耗生成额度，只有用户明确要求执行时才能调用。", { nodeIds: { type: "array", items: { type: "string" }, minItems: 1 }, resume: { type: "boolean" } }, ["nodeIds"]),
