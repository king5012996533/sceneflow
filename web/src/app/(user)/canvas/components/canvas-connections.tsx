...
export function ConnectionPath({
    ...
    return (
        <g>
            ...
            <path d={pathD} stroke={active || flowing ? theme.node.activeStroke : theme.node.muted} strokeWidth={active || flowing ? 2.5 : 2} strokeOpacity={flowing ? 0.26 : active ? 0.14 : 0.08} fill="none" style={{ pointerEvents: "none" }} />
            {/* 主线：挂载时 0.35s 画入 */}
            <path
                d={pathD}
                pathLength={1}
                ...
                strokeOpacity={flowing ? 1 : active ? 1 : 0.75}
                strokeDasharray={1}
                strokeDashoffset={1}
                fill="none"
                style={{ animation: "canvas-connection-draw 0.35s ease-out both", filter: active || flowing ? `drop-shadow(0 0 4px ${theme.node.activeStroke}44)` : undefined, pointerEvents: "none" }}
            />
