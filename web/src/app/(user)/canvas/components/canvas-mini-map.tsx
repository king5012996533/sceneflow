        ...
        <div className="absolute bottom-24 left-6 z-50 overflow-hidden rounded-lg border shadow-2xl backdrop-blur-sm" style={{ width, height, background: theme.toolbar.panel, borderColor: theme.toolbar.border }}>
            ...
            >
                {nodes.map((node) => {
                    ...
                    const color = node.type === CanvasNodeType.Image ? "#4e7c5d" : node.type === CanvasNodeType.Video ? "#a0713f" : node.type === CanvasNodeType.Audio ? "#9a7fa6" : node.type === CanvasNodeType.Config ? "#5c7fa3" : theme.node.muted;
                    return (
                        <div
                            key={node.id}
