...
export function CanvasConfigNodePanel({ node, isRunning, inputSummary, onConfigChange, onGenerate, onStop, onComposerToggle }: CanvasConfigNodePanelProps) {
    ...
    const credits = estimatedRequestCost(mode, config.model, { count: mode === "image" ? count : 1, videoSeconds: mode === "video" ? config.videoSeconds : undefined, vquality: mode === "video" ? config.vquality : undefined });
