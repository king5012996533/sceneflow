...
function MentionMenu({
    ...
    const stopCanvasInteraction = (event: React.PointerEvent | React.MouseEvent) => event.stopPropagation();
...
function ReferencePreview({ reference }: { reference: CanvasResourceReference }) {
    ...
    if (reference.kind === "video" && reference.previewUrl) return <video src={assetProxyUrl(upgradeInsecureMediaUrl(reference.previewUrl), "video")} className="size-9 rounded-md bg-black object-cover" muted preload="metadata" />;
...
function isPlainEnterKey(event: KeyboardEvent) {
