...
export default function CreditPackagesAdmin() {
    ...
    function renderRow(row: PackageRow, key: string, mode: "edit" | "draft") {
        ...
        return (
            <div
                key={key}
                className={`grid grid-cols-2 items-center gap-3 rounded-lg border px-3 py-2.5 md:grid-cols-[1fr_88px_96px_80px_72px_56px_auto] ${row.isActive ? "border-[#e2dfdc] bg-[#f9f7f5]" : "border-dashed border-[#e2dfdc] bg-[#f6f4f2] opacity-75"}`}
            >
                ...
                <div>
                    <div className="mb-0.5 text-[10px] text-[#726d67]">积分</div>
                ...
                <div>
                    <div className="mb-0.5 text-[10px] text-[#726d67]">价格（元）</div>
                ...
                <div>
                    <div className="mb-0.5 text-[10px] text-[#726d67]">赠送</div>
                ...
                <div>
                    <div className="mb-0.5 text-[10px] text-[#726d67]">排序</div>
                ...
                <div>
                    <div className="mb-0.5 text-[10px] text-[#726d67]">启用</div>
    ...
    return (
        <section className="rounded-2xl border border-[#e2dfdc] bg-[#ffffff] p-5 shadow-[0_8px_20px_rgba(35,28,20,0.05)]">
            ...
            <p className="mb-4 text-xs leading-5 text-[#726d67]">定价页 / 购买页实时读取启用中的套餐（停用即从页面下架）；改价不影响已生成的订单（金额为下单时快照）。</p>
            {loading ? (
                <div className="py-6 text-center text-sm text-[#726d67]">加载中…</div>
            ) : (
                <div className="space-y-2">
                    {rows.map((row) => renderRow(row, row.id, "edit"))}
                    {draft ? renderRow(draft, "draft", "draft") : null}
                    {!rows.length && !draft ? <div className="py-6 text-center text-sm text-[#a49f9a]">暂无套餐，点击右上角「新增套餐」创建。</div> : null}
                </div>
            )}
        </section>
    );
