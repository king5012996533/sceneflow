...
export function CredentialFormFields({ form, onChange, editMode }: CredentialFormFieldsProps) {
    ...
    return (
        <div className="space-y-3 py-2">
            ...
            <div>
                <div className="mb-1 text-sm text-[#2a3330]">绑定模型（逗号分隔；留空 = 全部）</div>
            ...
            <div>
                <div className="mb-1 text-sm text-[#2a3330]">优先级（越大越优先，同供应商多 Key 时生效）</div>
                <InputNumber className="w-full" min={0} value={form.priority} onChange={(value) => set({ priority: Number(value) || 0 })} />
            </div>
            <div>
                <div className="mb-1 text-sm text-[#2a3330]">逐模型能力标定（与前端画质 / 分辨率 / 比例 / 时长等一一对应）</div>
            ...
            <div>
                <div className="mb-1 text-sm text-[#2a3330]">逐模型积分定价（图片每张 / 视频每条 / 音频每次 / 文本每次；未配置 = 全局默认 → 内置草案）</div>
