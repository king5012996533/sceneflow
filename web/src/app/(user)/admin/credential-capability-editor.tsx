...
export function CredentialCapabilityEditor({ models, value, onChange, provider }: CredentialCapabilityEditorProps) {
    ...
    if (!models.length) {
        return (
            <div className="rounded-lg border border-dashed border-[#e2dfdc] px-3 py-4 text-center text-xs leading-5 text-[#726d67]">
                    ...
                    <div key={model} className="rounded-lg border border-[#e2dfdc] bg-[#f9f7f5]">
                        <div className="flex items-center justify-between gap-2 px-3 py-2">
                            <button type="button" className="flex min-w-0 flex-1 cursor-pointer items-center gap-2 text-left" onClick={toggle}>
                                ...
                                {enabled ? <span className="shrink-0 rounded bg-blue-50 px-1.5 py-0.5 text-[11px] text-blue-600">已标定</span> : <span className="shrink-0 text-[11px] text-[#726d67]">内置默认</span>}
                            </button>
                        ...
                        {open ? (
                            <div className="border-t border-[#e2dfdc] px-3 py-3">
                                {!supportsCapability ? (
                                    ...
                                    <div className="text-xs leading-5 text-[#726d67]">未启用：前端使用内置默认参数。打开开关后可按前端画质 / 分辨率 / 比例标定。</div>
                                )}
                            </div>
                        ) : null}
                    </div>
