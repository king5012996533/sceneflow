...
export function NavDrawer({ open, activeToolSlug, onClose }: NavDrawerProps) {
    ...
    const toolsBySlug = Object.fromEntries(navigationTools.map((tool) => [tool.slug, tool])) as Record<NavigationToolSlug, (typeof navigationTools)[number]>;
        ...
        >
            ...
            <div className="flex h-16 shrink-0 items-center justify-between border-b border-[#ded2c3] pl-5 pr-3.5">
                ...
                <button
                    ...
                    aria-label="关闭导航"
                    title="关闭导航"
                    className="flex size-8 shrink-0 items-center justify-center rounded-[9px] text-[#7a6d63] transition hover:bg-[#f1e3cf] hover:text-[#201914]"
                >
                    <X className="size-4" />
                </button>
            </div>
                                    ...
                                    return (
                                        <Link
                                            ...
                                            className={cn(
                                                ...
                                                active &&
                                                    "bg-[#f1e3cf] font-semibold text-[#201914] before:absolute before:left-0 before:top-1/2 before:h-5 before:w-[3px] before:-translate-y-1/2 before:rounded-r-[3px] before:bg-[#9b5b32]",
                                            )}
                                        >
                                            <span
                                                className={cn(
                                                    "flex size-[30px] shrink-0 items-center justify-center rounded-[9px] border border-[#ded2c3] bg-[#fffdf8] text-[#7a6d63]",
                                                    active && "border-transparent text-[#9b5b32]",
                                                )}
                                            >
            ...
            <div className="shrink-0 border-t border-[#ded2c3] px-3.5 pb-3.5 pt-3">
                {user ? (
                    <div className="flex items-center gap-2.5 px-1.5 pb-2.5 pt-1">
                        <span className="sf-serif flex size-8 shrink-0 items-center justify-center rounded-[10px] bg-[#9b5b32] text-[15px] font-semibold text-[#fffaf2]">
                        ...
                        <button
                            type="button"
                            onClick={() => {
                                void clearSession().finally(() => router.push("/canvas/login"));
                            }}
                            title="退出登录"
                            aria-label="退出登录"
                    ...
                    <div className="flex items-center gap-2 px-1.5 pb-2.5 pt-1">
                        <Link
                            href="/canvas/login"
                            onClick={onClose}
                            className="flex h-8 flex-1 items-center justify-center gap-1.5 rounded-[9px] border border-[#ded2c3] bg-[#fffdf8] text-xs font-semibold text-[#201914] transition hover:bg-[#f2ede4]"
                        >
                            <UserRound className="size-3.5" />
                            登录
                        </Link>
                        <Link
                            href="/canvas/register"
                            onClick={onClose}
                            className="flex h-8 flex-1 items-center justify-center gap-1.5 rounded-[9px] bg-[#9b5b32] text-xs font-semibold text-[#fffaf2] transition hover:bg-[#8a4f2b]"
                        >
                            注册
                        </Link>
                    </div>
