...
export function ReferralCard() {
    ...
    return (
        <section className="rounded-2xl border border-[#e2dfdc] bg-white/78 p-5 shadow-[0_20px_60px_rgba(51, 47, 42,0.06)]">
            ...
            {loading || !summary ? (
                <div className="py-2 text-sm text-[#726d67]">{loading ? "加载中…" : "邀请信息暂不可用"}</div>
            ) : (
                <>
                    <div className="grid gap-3 sm:grid-cols-3">
                        <div className="rounded-xl border border-[#ece5d8] bg-[#fbf7ef] px-4 py-3">
                            <div className="text-xs text-[#726d67]">已邀请好友</div>
                            <div className="mt-1 text-xl font-semibold text-[#23201c]">{summary.invitedCount} 人</div>
                        </div>
                        <div className="rounded-xl border border-[#ece5d8] bg-[#fbf7ef] px-4 py-3">
                            <div className="text-xs text-[#726d67]">累计返利</div>
                            <div className="mt-1 text-xl font-semibold text-[#23201c]">{summary.totalEarned} 积分</div>
                        </div>
                        <div className="rounded-xl border border-[#ece5d8] bg-[#fbf7ef] px-4 py-3">
                            <div className="text-xs text-[#726d67]">我的邀请码</div>
                            <div className="mt-1 flex items-center justify-between gap-2">
                                <span className="sf-mono text-lg font-semibold tracking-wider text-[#23201c]">{summary.referralCode}</span>
                    ...
                    <div className="mt-3 flex items-center gap-2 rounded-xl border border-[#ece5d8] bg-white px-3 py-2">
                        <input readOnly value={inviteLink} className="sf-mono min-w-0 flex-1 bg-transparent text-xs text-[#726d67] outline-none" onFocus={(event) => event.target.select()} />
                        <Button size="small" className="!rounded-lg !border-[#a0713f] !bg-[#a0713f] !text-white" onClick={() => copy(inviteLink, "邀请链接已复制")}>
                            复制链接
                        </Button>
                    </div>
                    <div className="mt-3 flex items-start gap-1.5 text-xs leading-5 text-[#726d67]">
