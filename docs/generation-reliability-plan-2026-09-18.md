...
| 阶段 | 内容 | 状态 | 完成时间 |
|---|---|---|---|
| 0 | 止血 + 账对得上（取消不扔图、丢弃即日志、对账字段与日报） | 🟡 代码完成，待线上验证 | 2026-09-18 |
...
- [ ] 真机复验「取消不扔图」：起真任务 → 1.5 秒后取消 → 等上游返回 → 断言：任务 `cancelled`、已退款、**归档目录里有该任务的成品**、记录页能看到并下载。
- [ ] 真机复验记录页显示（桌面 + 手机视口）。
- [ ] 日报真机跑一次：`curl` 内部接口，核对四本账数字与库里逐条对得上（抽样 10 条）。
