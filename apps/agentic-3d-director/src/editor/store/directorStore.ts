...
export interface DirectorUiState {
  ...
  trajectoryDrawingObjectId: string | null;
  trajectoryDraftPoints: [number, number, number][];
}
...
export interface DirectorActions {
  ...
  setTimelineCurrentTime: (timeSec: number) => void;
  setTimelinePlaying: (playing: boolean) => void;
  setTimelineExporting: (exporting: boolean) => void;
  setTimelineDuration: (durationSec: number) => void;
  setTimelineLoop: (loop: boolean) => void;
  selectTimelineTrack: (trackId: string | null, keyframeId?: string | null) => void;
  createObjectTrajectory: (
    objectId: string,
    ...
    options?: {
      ...
      source?: "manual" | "preset" | "agent";
      color?: string;
    }
  ) => string | null;
  updateTimelineTrack: (
    trackId: string,
    patch: Partial<Pick<DirectorTimelineTrack, "enabled" | "color" | "orientToPath" | "motion">>
  ) => void;
  updateTimelineKeyframe: (
    trackId: string,
    keyframeId: string,
    patch: Partial<Omit<DirectorTimelineKeyframe, "id">>
  ) => void;
  deleteTimelineTrack: (trackId: string) => void;
  beginTrajectoryDrawing: (objectId: string) => void;
  addTrajectoryDraftPoint: (position: [number, number, number]) => void;
  finishTrajectoryDrawing: (durationSec?: number) => string | null;
  cancelTrajectoryDrawing: () => void;
...
const DEFAULT_UI_STATE: DirectorUiState = {
  ...
  trajectoryDrawingObjectId: null,
  trajectoryDraftPoints: [],
};
...
function migrateDirectorProject(project: DirectorProject): DirectorProject {
  return {
    ...project,
    timeline: normalizeDirectorTimeline(project.timeline),
...
function readPersistedDirectorState(options: DirectorStateOptions = {}): DirectorState | null {
  ...
  try {
    ...
    return {
      ...
      timelinePanelOpen: Boolean(state.timelinePanelOpen),
      timelineCurrentTime: Math.max(0, Number(state.timelineCurrentTime) || 0),
      timelinePlaying: false,
      timelineExporting: false,
      selectedTimelineTrackId:
        typeof state.selectedTimelineTrackId === "string" ? state.selectedTimelineTrackId : null,
      selectedTimelineKeyframeId:
        typeof state.selectedTimelineKeyframeId === "string" ? state.selectedTimelineKeyframeId : null,
      trajectoryDrawingObjectId: null,
      trajectoryDraftPoints: [],
...
export function createDefaultDirectorProject({
  ...
  return {
    ...
    timeline: createDefaultDirectorTimeline(),
  };
}
  ...
  return {
    ...
    toggleTimelinePanel: () =>
      commitUiMutation((state) => ({
        ...state,
        timelinePanelOpen: !state.timelinePanelOpen,
        timelinePlaying: state.timelinePanelOpen ? false : state.timelinePlaying,
      })),
    setTimelinePanelOpen: (open) =>
      commitUiMutation((state) => ({
        ...state,
        timelinePanelOpen: open,
        timelinePlaying: open ? state.timelinePlaying : false,
      })),
    setTimelineCurrentTime: (timeSec) =>
      set((state) => {
        const currentState = state as DirectorRuntimeState;
        const timeline = normalizeDirectorTimeline(currentState.project.timeline);
        const nextTime = Number.isFinite(timeSec)
          ? Math.min(timeline.durationSec, Math.max(0, timeSec))
          : currentState.timelineCurrentTime;

        return {
          ...currentState,
          timelineCurrentTime: nextTime,
        };
      }),
    setTimelinePlaying: (playing) =>
      set((state) => ({
        ...(state as DirectorRuntimeState),
        timelinePlaying: (state as DirectorRuntimeState).timelineExporting ? false : playing,
        timelinePanelOpen: playing ? true : (state as DirectorRuntimeState).timelinePanelOpen,
      })),
    setTimelineExporting: (exporting) =>
      set((state) => ({
        ...(state as DirectorRuntimeState),
        timelinePlaying: exporting ? false : (state as DirectorRuntimeState).timelinePlaying,
        timelineExporting: exporting,
      })),
    setTimelineDuration: (durationSec) =>
      commitMutation((state) => {
        const timeline = normalizeDirectorTimeline(state.project.timeline);
        const nextDuration = Math.min(
          MAX_TIMELINE_DURATION_SEC,
          Math.max(MIN_TIMELINE_DURATION_SEC, Number.isFinite(durationSec) ? durationSec : timeline.durationSec)
        );

        return {
          ...state,
          timelineCurrentTime: Math.min(state.timelineCurrentTime, nextDuration),
          project: {
            ...state.project,
            timeline: {
              ...timeline,
              ...
              tracks: timeline.tracks.map((track) => ({
                ...
                keyframes: track.keyframes
                  .map((frame) => ({ ...frame, timeSec: Math.min(frame.timeSec, nextDuration) }))
    ...
    setTimelineLoop: (loop) =>
      commitMutation((state) => ({
        ...
        project: {
          ...state.project,
          timeline: {
            ...normalizeDirectorTimeline(state.project.timeline),
            loop,
          },
        },
      })),
    selectTimelineTrack: (trackId, keyframeId = null) =>
      set((state) => ({
        ...(state as DirectorRuntimeState),
        selectedTimelineTrackId: trackId,
        selectedTimelineKeyframeId: keyframeId,
      })),
    createObjectTrajectory: (objectId, preset, options = {}) => {
      ...
      commitMutation((state) => {
        ...
        const track = createTrajectoryTrack({
          object,
          camera,
          durationSec: options.durationSec ?? timeline.durationSec,
          preset,
          waypoints: options.waypoints,
          radius: options.radius,
          width: options.width,
          depth: options.depth,
          orientToPath: options.orientToPath,
        ...
        return {
          ...
          project: {
            ...
            timeline: {
              ...timeline,
              durationSec: Math.max(timeline.durationSec, options.durationSec ?? 0),
              tracks: [...timeline.tracks.filter((item) => item.objectId !== objectId), track],
            },
          },
        };
      });
    ...
    updateTimelineKeyframe: (trackId, keyframeId, patch) =>
      commitMutation((state) => {
        const timeline = normalizeDirectorTimeline(state.project.timeline);
            ...
            timeline: {
              ...
              tracks: timeline.tracks.map((track) =>
                track.id === trackId
                  ? {
                      ...track,
                      preset: track.circle && patch.transform ? "custom" : track.preset,
                      circle: track.circle && patch.transform ? undefined : track.circle,
                      keyframes: track.keyframes
                        .map((frame) =>
                          frame.id === keyframeId
                            ? {
                                ...
                                id: frame.id,
                                timeSec:
                                  patch.timeSec === undefined
                                    ? frame.timeSec
                                    : Math.min(timeline.durationSec, Math.max(0, patch.timeSec)),
                                transform: patch.transform ?? frame.transform,
                              }
                            : frame
                        )
                        .sort((a, b) => a.timeSec - b.timeSec),
    ...
    deleteTimelineTrack: (trackId) =>
      commitMutation((state) => {
        const timeline = normalizeDirectorTimeline(state.project.timeline);
        if (!timeline.tracks.some((track) => track.id === trackId)) return state;

        return {
          ...state,
          selectedTimelineTrackId: state.selectedTimelineTrackId === trackId ? null : state.selectedTimelineTrackId,
          selectedTimelineKeyframeId: state.selectedTimelineTrackId === trackId ? null : state.selectedTimelineKeyframeId,
          project: {
            ...state.project,
            timeline: {
              ...timeline,
    ...
    beginTrajectoryDrawing: (objectId) => {
      ...
      set({
        ...
        timelinePanelOpen: true,
        timelinePlaying: false,
        trajectoryDrawingObjectId: objectId,
        trajectoryDraftPoints: [[...object.transform.position]],
      });
    },
    addTrajectoryDraftPoint: (position) => {
      const state = get() as DirectorRuntimeState;
      const object = state.trajectoryDrawingObjectId
        ? state.project.objects.find((item) => item.id === state.trajectoryDrawingObjectId)
        : null;
      if (!object || !position.every(Number.isFinite)) return;
      const nextPoint: [number, number, number] = [position[0], object.transform.position[1], position[2]];
      const previousPoint = state.trajectoryDraftPoints[state.trajectoryDraftPoints.length - 1];
      if (previousPoint && Math.hypot(nextPoint[0] - previousPoint[0], nextPoint[2] - previousPoint[2]) < 0.05) {
        return;
      }

      set({
        ...state,
        trajectoryDraftPoints: [...state.trajectoryDraftPoints, nextPoint],
      });
    },
    finishTrajectoryDrawing: (durationSec) => {
      const state = get() as DirectorRuntimeState;
      const objectId = state.trajectoryDrawingObjectId;
      if (!objectId || state.trajectoryDraftPoints.length < 2) return null;
      const timeline = normalizeDirectorTimeline(state.project.timeline);
      const points = state.trajectoryDraftPoints.map((position) => ({ position }));
      const trackId = (get() as DirectorStore).createObjectTrajectory(objectId, "custom", {
        durationSec: durationSec ?? timeline.durationSec,
        waypoints: points,
        source: "manual",
      });
      const latest = get() as DirectorRuntimeState;
      set({
        ...latest,
        trajectoryDrawingObjectId: null,
        trajectoryDraftPoints: [],
      ...
      commitMutation((state) => {
        ...
        return {
          ...
          project: {
            ...
            timeline: {
              ...normalizeDirectorTimeline(state.project.timeline),
              tracks: normalizeDirectorTimeline(state.project.timeline).tracks.filter(
