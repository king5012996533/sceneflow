...
import { applyTimelineToCamera } from "../timeline/animationTimeline";
...
export function DirectorCanvas() {
  ...
  const animatedActiveCamera = useMemo(() => {
    if (!activeCamera) return undefined;
    const cameraObject = objects.find(
      (item) => item.kind === "camera" && item.linkedCameraId === activeCamera.id
    );
    return applyTimelineToCamera(activeCamera, cameraObject, timeline, timelineCurrentTime);
  }, [activeCamera, objects, timeline, timelineCurrentTime]);
  const activeCameraView = animatedActiveCamera
    ? getCameraViewSnapshotFromShot(animatedActiveCamera)
    : undefined;
    ...
    <div className="canvas-frame">
      <div className="director-canvas" data-testid="director-canvas">
        ...
        >
          ...
          {showViewportGrid && !timelineExporting ? (
            <Grid
              cellThickness={0}
              fadeDistance={80}
              infiniteGrid
          ...
          <CanvasCaptureBridge
            activeCamera={animatedActiveCamera}
