...
export function ViewportToolbar({
  ...
  const actions: ToolbarAction[] = [
    ...
    { label: "动画时间轴", icon: Clock3, active: timelinePanelOpen, onClick: toggleTimelinePanel },
  ];

  function renderActionButton(action: ToolbarAction) {
    const Icon = action.icon;
    const active = action.mode ? transformMode === action.mode : Boolean(action.active);
    const pressable = Boolean(action.mode) || action.active !== undefined;
