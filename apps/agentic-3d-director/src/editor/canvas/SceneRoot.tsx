...
import { TimelinePathOverlay } from "../timeline/TimelinePathOverlay";
...
export function SceneRoot() {
  ...
  const timelineCurrentTime = useDirectorStore((state) => state.timelineCurrentTime);
  const timelinePlaying = useDirectorStore((state) => state.timelinePlaying);
  const timelineExporting = useDirectorStore((state) => state.timelineExporting);
  ...
  const animatedObjects = useMemo(
    () => objects.map((item) => applyTimelineToObject(item, timeline, timelineCurrentTime)),
    [objects, timeline, timelineCurrentTime]
  );
  const animatedCameras = useMemo(
    () =>
      cameras.map((camera) =>
        applyTimelineToCamera(
          camera,
          objects.find((item) => item.kind === "camera" && item.linkedCameraId === camera.id),
          timeline,
          timelineCurrentTime
        )
      ),
    [cameras, objects, timeline, timelineCurrentTime]
  );
  ...
  return (
    ...
    >
      ...
      {!timelineExporting ? <TimelinePathOverlay groundHeight={scene.groundHeight} /> : null}
      {animatedObjects
        ...
        .map((item) => {
          ...
          return (
            <ObjectSceneNode
              ...
              transformMode={transformMode}
              transformable={!item.locked && !timelinePlaying && !timelineExporting && !getTimelineTrack(timeline, item.id)}
              translationSnap={translationSnap}
              onSelect={handleObjectSelect}
            />
          );
        })}
      {Array.from(new Set(animatedObjects.map((item) => item.crowdId).filter((item): item is string => typeof item === "string"))).map(
        (crowdId) => (
          <CrowdTransformRig
            key={crowdId}
            crowdId={crowdId}
      ...
      {viewMode === "director" && !timelineExporting
        ? animatedCameras
            ...
            .map(({ camera, object }) => (
              <ViewportCameraRig
                ...
                transformMode={transformMode}
                transformable={Boolean(object && !object.locked && !timelinePlaying && !timelineExporting && !getTimelineTrack(timeline, object.id))}
                translationSnap={translationSnap}
              />
            ))
        : null}
